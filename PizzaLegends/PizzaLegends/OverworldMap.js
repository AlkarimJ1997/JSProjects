class OverworldMap {
    constructor(config) {
        ...
        // Flag for if a cutscene is playing
        this.isCutscenePlaying = false;

        // Back reference for the overworld
        this.overworld = null;

        // Flag for if the game is currently paused
        this.isPaused = false;
    }
    ...
    // Method to mount game objects to the map
    mountObjects() {
        Object.keys(this.gameObjects).forEach(key => {
            const gameObject = this.gameObjects[key];
            
            // Set the game object's ID and mount it to the map
            gameObject.id = key;
            gameObject.mount(this);
        });
    }

    // Method to start global cutscene
    async startCutscene(events) {
        this.isCutscenePlaying = true;

        // Start loop of async events (waiting for each event to finish)
        for (let i = 0; i < events.length; i++) {
            const eventHandler = new OverworldEvent({ map: this, event: events[i] });

            const result = await eventHandler.init();

            // If we lost the battle, stop the cutscene (so no flags are set)
        ...
        Object.values(this.gameObjects).forEach(object => {
            ...
            // If object is already retrying, don't do anything
            ...
            // If walking
            if (current && current.type === "walk" && object.movingProgressRemaining === 0) {
                object.doBehaviorEvent(this);
            }
        })
    }

    // Method to check for action cutscenes (NPCs) based on the position of the hero
    checkForActionCutscene() {
        const hero = this.gameObjects.hero;
        const nextCoords = utils.nextPosition(hero.x, hero.y, hero.direction);

        // Check if there is an NPC in front of the hero
        const match = Object.values(this.gameObjects).find(gameObject => {
            return `${gameObject.x},${gameObject.y}` === `${nextCoords.x},${nextCoords.y}`
        });

        // Check if the NPC has something to say (assuming no other cutscene is playing)
        if (!this.isCutscenePlaying && match && match.talking.length) {
            // Find the appropriate cutscene or event behavior that should play
            const relevantScenario = match.talking.find(scenario => {
                return (scenario.required || []).every(sf => {
                    // Check if the appropriate story flags are present
                    return playerState.storyFlags[sf];
                })
            })
            
            // Start the cutscene or event behavior (if one is appropriate)
            relevantScenario && this.startCutscene(relevantScenario.events);
        }
    }

    // Method to check for footstep cutscenes based on the position of the hero
    checkForFootstepCutscene() {
        const hero = this.gameObjects.hero;

        // Check if the hero is on a cutscene space
        const match = this.cutsceneSpaces[`${hero.x},${hero.y}`];

        // Check if the event is not disqualified (for one time events)
        const isValid = match && (match[0].disqualify || []).every(sf => {
            // Check if the disqualified flags are already present (meaning the event has already happened)
            return !playerState.storyFlags[sf];
        })

        // Check if there is a cutscene to play (assuming no other cutscene is playing)
        if (!this.isCutscenePlaying && match && isValid) {
            // Start the cutscene
            this.startCutscene(match[0].events)
        }
    }

    // Method to add wall whenever game object is created
    addWall(x, y) {
        this.walls[`${x},${y}`] = true;
    }

    // Method to remove wall whenever game object is destroyed
    removeWall(x, y) {
        delete this.walls[`${x},${y}`]
    }

    // Method to move wall whenever game object (NPC) is moved
    ...
    Street: {
                ...
                talking: [
                    {
                        events: [
                            ...
                            // { type: "follow", follower: "hero", who: "streetNpcA", direction: "left" },
                            // { type: "follow", follower: "hero", who: "streetNpcA", direction: "left" },
                            // { type: "follow", follower: "hero", who: "streetNpcA", direction: "left" },
                            // { type: "follow", follower: "hero", who: "streetNpcA", direction: "down" }
                        ]
                    }
                ]
            }),
        ...
        cutsceneSpaces: {
            [utils.asGridCoord(5, 13)]: [
                {
                    events: [
                        { type: "goToNPC", who: "hero", target: "streetNpcA" },
                        // { type: "goToNPC", who: "streetNpcA", target: "hero" },
                        { type: "jump", who: "streetNpcA" },
                        { type: "textMessage", text: "Welcome to Anchovy Avenue!" },
                    ]
                }
            ]
            ,
                ...
                talking: [
                    {
                        events: [
                            ...
                            { type: "battle", enemyId: "greenKitchenBattle", arena: "green-kitchen" },
                        ]
                    }
                ]
            }),
